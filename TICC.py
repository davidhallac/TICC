...
num_blocks = 11## window_size + 1
maxIters = 100 ##number of Iterations of the smoothening + clustering algo
switch_penalty = 20## smoothness penalty
seg_len = 300##segment-length : used in confusion matrix computation
lam_sparse = 11e-2##sparsity parameter

maxClusters = 3## Number of clusters + 1
...
def computeNetworkAccuracy(matching,train_cluster_inverse, num_clusters):
	...
	for cluster in xrange(num_clusters):
		...
		# (nrow,ncol) = matched_cluster_cov.shape
...
for iters in xrange(maxIters):
	...
	if iters == 0:
		...
		print "length of test index is:", len(test_idx)
		...
		print "GMM TRUE confusion MATRIX:\n", true_confusion_matrix_g					
		true_confusion_matrix_k = compute_confusion_matrix(num_clusters,clustered_points_kmeans,sorted_training_idx)
		print "KMEANS TRAINING CONFUSION MATRIX:\n", true_confusion_matrix_k
	...
	print "counter of total train cluster and test cluster points is:", counter
